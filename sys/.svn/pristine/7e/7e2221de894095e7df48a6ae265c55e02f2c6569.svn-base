...
<body>
	...
	<div class="main_content">
				...
				<div class="menu_line">
					...
					<c:forEach items="${menu.subMenu}" var="sub">
						...
						<c:choose>
							...
							<c:if test="${fn:indexOf(sub.menuUrl, '&') > -1}">
								<li><a href="${sub.menuUrl }&menuId=${sub.menuId}" target="mainFrame">${sub.menuName }</a></li>
							</c:if>
							<c:if test="${fn:indexOf(sub.menuUrl, '&') < 0}">
								<li><a href="${sub.menuUrl }?menuId=${sub.menuId}" target="mainFrame">${sub.menuName }</a></li>
							</c:if>
							</c:when>
							<c:otherwise>
		...
		<div class="content_right">
			...
			<div class="sys_bottom"> Copyright &copy; 2014 微推渠道运营系统</div>
		</div>
	</div>
