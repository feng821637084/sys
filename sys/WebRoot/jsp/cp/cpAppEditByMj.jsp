...
<body>
	<form action="saveByMj.html" name="cpProdectForm" id="cpProdectForm" target="result" method="post" onsubmit="return checkInfo();">
	...
	<table border="0" cellpadding="0" cellspacing="0">
		<tr class="info">
			<th >产品名称:</th>
			<td><input type="text" readonly="readonly" name="appName" id="appName" class="input_txt" value="${cpProdect.appName }"/></td>
		</tr>
		<tr class="info">
			<!-- 
			<th>所属CP:</th>
			<td>
			    <select readonly="readonly" name="pDataId" id="pDataId" style="vertical-align: middle;" onchange="setPData(this[selectedIndex].innerHTML);">
					<option value="">请选择</option>
					<c:forEach items="${cpList}" var="cp">
					<option value="${cp.id}" <c:if test="${cp.id==cpProdect.pDataId}">selected</c:if>>${cp.cpContact }@${cp.cpCompanyName }</option>
					</c:forEach>
				</select>
			</td>
			 -->
		...
		<tr class="info">
			<th>CP价格:</th>
			<td><input readonly="readonly" type="text" name="cpPrice" id="cpPrice" class="input_txt" value="${cpProdect.cpPrice }"/></td>
		</tr>
		<tr class="info">
			<th>CP后台:</th>
			<td><textarea  cols="15" id="appHtUrl" name="appHtUrl" rows="15" style="width: 400px; height: 100px;">${cpProdect.appHtUrl }</textarea></td>
		</tr>
		...
		<tr class="info">
			<th>负责人:</th>
			<td><input readonly="readonly" type="text" name="appFzr" id="appFzr" class="input_txt" value="${cpProdect.appFzr }"/></td>
		</tr>
		<tr class="info">
			<th>结算周期:</th>
			<td>
		...
		<tr class="info">
			<th>分配渠道:</th>
			<td><input type="text" name="qdOperator" id="qdOperator" class="input_txt" value="${cpProdect.qdOperator }" readonly = "readonly" />
				<div>
		...
		<tr class="info">
			<th>所属渠道价格:</th>
			<td><input type="text" name="qdPrice" id="qdPrice" class="input_txt" value="${cpProdect.qdPrice }"/></td>
		</tr>
	</table>
	</form>
