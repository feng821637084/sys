...
<body>
	<form action="userDialog.html" method="post" name="userForm" id="userForm">
	<div class="search_div">
		...
		<a href="javascript:search('${pUserId }');" class="myBtn"><em>查询</em></a>
	</div>
	<table width="100%" border="0" cellpadding="0" cellspacing="0" class="main_table">
		...
		<c:choose>
			<c:when test="${not empty userList}">
					...
					<td><a href="javascript:qdRelventUser(${user.userId });">关联该用户</a></td>
					</tr>
					</c:forEach>
			</c:when>
			<c:otherwise>
	...
	<script type="text/javascript">
		...
		function search(pUserId){
		    var url = "user.html?pUserId="+pUserId;
				$.get(url,function(data){
			});
		}
		...
		function qdRelventUser(userId){
		    var pUserId = '${pUserId}' ;
		    //if(confirm("确定关联此用户吗？")){
				var url = "user/relevantQDUser.html?pUserId="+pUserId+"&userId="+userId;
				$.get(url,function(data){
					if(data=="success"){
						parent.success();
					}
